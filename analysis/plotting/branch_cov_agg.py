...
for trial_csv in data_path.glob("trial_*.csv"):
    ...
    trial_df.loc[trial_df["errors"] > 0, "branch_cov"] = None
    coverage = trial_df["branch_cov"]

    df = pd.DataFrame({"branch_cov": coverage}).assign(trial_id=trial_id) \
...
sns.lineplot(data=plot_df, x="iteration", y="mean", marker="o", label=r"$\bar{b}_k$ - mean Branch coverage")
sns.lineplot(data=plot_df, x="iteration", y="median", linestyle="--", label=r"$\tilde{b}_k$ - median Branch coverage")
...
plt.tight_layout()
plt.savefig(plots_path / "branch_coverage_agg.svg")
print(f"✅ Plot for aggregated Branch coverage generated: {(plots_path / "branch_coverage_agg.svg")}")
